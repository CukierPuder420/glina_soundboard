...
import '../funkcje/przycisk_z_dzwiekiem.dart';
...
class _EkranGlownyState extends State<EkranGlownyState> {
  final detector = ShakeDetector.autoStart(onPhoneShake: () {
    arrr();
  });
    ...
    return Scaffold(
      ...
      body: Builder(
        builder: (context) => CustomScrollView(
          slivers: <Widget>[
            SliverAppBar(
              title: const Text('Glina Soundboard'),
              backgroundColor: Colors.greenAccent,
              expandedHeight: 300,
              floating: true,
              pinned: true,
              flexibleSpace: FlexibleSpaceBar(background: Image.asset('assets/zdjecia/morze.jpg', fit: BoxFit.cover)),
              actions: <Widget>[
                Container(
                  child: Row(
                    children: <Widget>[
                      ...
                      const Icon(MdiIcons.fire, color: Colors.deepOrange),
                    ],
                  ),
                ),
                IconButton(
                  ...
                  onPressed: () {
                    Navigator.push(
                        context, PageTransition<Widget>(child: Ulubione(), type: PageTransitionType.topToBottom, duration: const Duration(milliseconds: 150)));
                  },
                ),
              ],
            ),
